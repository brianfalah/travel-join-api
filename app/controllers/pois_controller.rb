class PoisController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      if params[:latitude] && params[:longitude] && params[:max_distance]
        pois = Poi.near([params[:latitude], params[:longitude]], params[:max_distance], :units => :km). # puntos a max_distance km
        ...
        if params['groups_ids']
          #traemos solo pois pertenecientes a esos grupos
          pois = pois.joins(:groups_pois).where("groups_pois.group_id IN(?)", eval(params['groups_ids']))
        end
